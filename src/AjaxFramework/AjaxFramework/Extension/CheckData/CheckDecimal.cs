...
namespace AjaxFramework
{
    ...
    /// 检查小数
    /// </summary>
    internal class CheckDecimal:CheckDataStrategy
    {
        public override bool CheckData()
        {
            ...
            {
                #region 检查是否允许为空
                if (!Regex.IsMatch(base.CurrentData.Value, @"^\d*[\.]?\d*$") || base.CurrentData.Nullable)
                {
                    //不是有效的数字
                ...
                #region 验证最大最小值
                double digit = Convert.ToDouble(base.CurrentData.Value);
