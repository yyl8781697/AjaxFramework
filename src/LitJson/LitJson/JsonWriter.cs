...
namespace LitJson
{
    ...
    public class JsonWriter
    {
        ...
        /// <summary>
        /// Whether to convert the chinese characters to unicode
        /// </summary>
        ...
        /// <summary>
        /// Whether the key is to be convert to camel type
        /// </summary>
        public bool ToCamelKey
        {
            get {
                return this.toCamelKey;
            }
            set {
                toCamelKey = true;
            }
        ...
        private void PutString (string str)
        {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (this.toUnicode)
                {
                    ...
                    {
                        writer.Write(@"\u" + ((int)str[i]).ToString("x"));
                        continue;
                    }
                }
                writer.Write (str[i]);
        ...
        private string ToCamel(string str, string separator)
        {
            ...
            str = regex.Replace(str, (m) =>
            {
                ...
                {
                    //if the match value is start with separator char
                    ret = m.Value.Substring(1).ToLower();//cut the separator char and convert to entity str to lower
                    ret = ret.Substring(0, 1).ToUpper() + ret.Substring(1);//convert the first char to upper
                }
                else
                {
                    ret=m.Value.ToLower();
